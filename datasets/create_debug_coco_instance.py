#!/usr/bin/env python3
# Creates a 1 image version of the COCO instance dataset for debugging purposes
import json
import os

def create_new_ds(original_json, oneimage_json, image_ids_keep):
    """
    Create new 1-image dataset from existing COCO instance segmentation dataset.
    Args:
...
if __name__ == "__main__":
    ...
    split = "val2017"
    create_new_ds(os.path.join(dataset_dir, "annotations/instances_{}.json".format(split)),
                  os.path.join(dataset_dir, "annotations/single_image_instance_debug_{}.json".format(split)),
                  image_ids_keep=[397354]) # Chosen as it has decent content and some overlapping instances

    create_new_ds(os.path.join(dataset_dir, "annotations/instances_{}.json".format(split)),
                  os.path.join(dataset_dir, "annotations/multi_image_instance_debug_{}.json".format(split)),
                  image_ids_keep=[18380, 31296, 191845, 194875, 89648])

# Example code for finding COCO images with the most instances
# import json
#
# with open('/usr/local/data/msmith/APL/Datasets/coco/annotations/instances_val2017.json', 'r') as f:
#     original = json.load(f)
#
# annotation_dict = {}
# for i in original['annotations']:
...
#     annotation_dict[img_id].append(i['id'])
#
